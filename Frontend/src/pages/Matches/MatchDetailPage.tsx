...
import { exportScorecardPDF } from '../../services/pdfService'
...
function downloadPDFScorecard(match: Match) {
  const mapBatsmen = (inn: NonNullable<Match['innings1']> | undefined) => {
    if (!inn) return []
    return (inn.batters || []).map(b => ({
      name: b.playerName,
  ...
  const mapBowlers = (inn: NonNullable<Match['innings1']> | undefined) => {
    if (!inn) return []
...
export default function MatchDetailPage({ isPublic = false }: MatchDetailPageProps) {
  ...
  // ── Toss Wizard state ──────────────────────────────────────────────────────
  const [showTossWizard,  setShowTossWizard]  = useState(false)
  const [tossStep,        setTossStep]        = useState(0)
  const [tossWinnerId,    setTossWinnerId]    = useState('')
  const [tossDecision,    setTossDecision]    = useState<'bat' | 'field' | ''>('')
  const [tossStartLoading, setTossStartLoading] = useState(false)

  function closeTossWizard() {
    setShowTossWizard(false)
    setTossStep(0)
    setTossWinnerId('')
    setTossDecision('')
  }
  ...
  // ── Toss Wizard confirm ────────────────────────────────────────────────────
  async function handleTossConfirm() {
    if (!match || !tossWinnerId || !tossDecision) return
    setTossStartLoading(true)
    try {
      // 1. Save toss decision to Firestore
      await updateMatch(match.id, {
        tossWinnerId,
        tossDecision: tossDecision as 'bat' | 'field',
      })
      // 2. Mark match as live
      await startMatch(match.id)
      showToast('Match started! 🏏 Setting up live scoring…', 'success')
      // 3. Reset wizard
      setShowTossWizard(false)
      setTossStep(0)
      // 4. Navigate to live scoring
      navigate(`/matches/${match.id}/live`)
    } catch {
      showToast('Failed to start match. Try again.', 'error')
    } finally {
      setTossStartLoading(false)
    }
  }
  ...
  const content = (
    <>
      ...
      <div className="match-section share-card">
        ...
        {match.status === 'completed' && (
          <button
            ...
            style={{ marginTop: 8 }}
            onClick={() => downloadPDFScorecard(match)}
          >
      ...
      {/* Admin actions — guided Toss Wizard + live scoring */}
      {isOwner && (
        <>
          {/* UPCOMING: show toss wizard button */}
          {match.status === 'upcoming' && (
            <div className="match-admin-bar" style={{ marginTop: 24, flexDirection: 'column', gap: 12 }}>
              <button
                id="start-toss-btn"
                className="team-btn team-btn--primary"
                style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', fontSize: 16, padding: '14px 28px' }}
                onClick={() => setShowTossWizard(true)}
              >
                🪙 Do Toss &amp; Start Match
              </button>
              <p style={{ color: '#64748b', fontSize: 12, textAlign: 'center', margin: 0 }}>
                Toss karo, bat/bowl choose karo, phir live scoring shuru hogi
              </p>
            </div>
          )}

          {/* LIVE: open live scoring */}
          {match.status === 'live' && (
            <div className="match-admin-bar" style={{ marginTop: 24, flexDirection: 'column', gap: 8 }}>
              <button
                id="live-score-btn"
                className="team-btn team-btn--primary"
              ...
              {match.tossWinnerId && match.tossDecision && (
                <div className="match-toss-info" style={{ textAlign: 'center' }}>
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 28 }}>
              {(['Toss', 'Bat / Bowl', 'Start!'] as const).map((label, i) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{
                    width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 12, fontWeight: 800,
                    background: tossStep > i ? '#22d3ee' : tossStep === i ? 'rgba(245,158,11,0.9)' : 'rgba(255,255,255,0.1)',
                    color: tossStep >= i ? '#0f172a' : '#64748b',
                    transition: 'all 0.3s',
                  }}>{tossStep > i ? '✓' : i + 1}</div>
                  <span style={{ fontSize: 11, color: tossStep === i ? '#f59e0b' : '#475569', fontWeight: 600 }}>{label}</span>
                  {i < 2 && <div style={{ width: 20, height: 1, background: tossStep > i ? '#22d3ee' : 'rgba(255,255,255,0.1)' }} />}
                </div>
              ))}
            </div>
            ...
            {tossStep === 0 && (
              <div style={{ textAlign: 'center' }}>
                ...
                <h2 style={{ color: '#f1f5f9', fontSize: 22, fontWeight: 900, margin: '0 0 6px' }}>Who Won the Toss?</h2>
                <p style={{ color: '#64748b', fontSize: 13, margin: '0 0 24px' }}>Coin flip karo aur winner select karo</p>
                <div style={{ display: 'flex', gap: 12, flexDirection: 'column' }}>
                  <button
                    id="toss-team1-btn"
                    className="team-btn team-btn--outline"
                    style={{ fontSize: 16, padding: '14px', borderColor: tossWinnerId === match.team1Id ? '#f59e0b' : undefined, background: tossWinnerId === match.team1Id ? 'rgba(245,158,11,0.12)' : undefined }}
                    onClick={() => setTossWinnerId(match.team1Id)}
                  >
                    {match.team1Logo} {match.team1Name}
                  </button>
                  <button
                    id="toss-team2-btn"
                    className="team-btn team-btn--outline"
                    style={{ fontSize: 16, padding: '14px', borderColor: tossWinnerId === match.team2Id ? '#f59e0b' : undefined, background: tossWinnerId === match.team2Id ? 'rgba(245,158,11,0.12)' : undefined }}
                    onClick={() => setTossWinnerId(match.team2Id)}
                  >
                    {match.team2Logo} {match.team2Name}
                  </button>
                </div>
            ...
            {tossStep === 1 && (
              <div style={{ textAlign: 'center' }}>
                ...
                <h2 style={{ color: '#f1f5f9', fontSize: 22, fontWeight: 900, margin: '0 0 6px' }}>
                  {tossWinnerId === match.team1Id ? match.team1Name : match.team2Name} Elected to…
                </h2>
                <p style={{ color: '#64748b', fontSize: 13, margin: '0 0 24px' }}>
                  Toss winner kya karna chahta hai?
                </p>
                <div style={{ display: 'flex', gap: 12 }}>
                  <button
                    id="toss-bat-btn"
                    className="team-btn team-btn--outline"
                    style={{ flex: 1, fontSize: 18, padding: '18px', borderColor: tossDecision === 'bat' ? '#22d3ee' : undefined, background: tossDecision === 'bat' ? 'rgba(34,211,238,0.1)' : undefined }}
                  ...
                  <button
                    id="toss-bowl-btn"
                    className="team-btn team-btn--outline"
                    style={{ flex: 1, fontSize: 18, padding: '18px', borderColor: tossDecision === 'field' ? '#a78bfa' : undefined, background: tossDecision === 'field' ? 'rgba(167,139,250,0.1)' : undefined }}
                ...
                {tossDecision && (
                  <div style={{ marginTop: 16, padding: '10px 16px', background: 'rgba(34,211,238,0.06)', border: '1px solid rgba(34,211,238,0.2)', borderRadius: 10, fontSize: 13, color: '#94a3b8' }}>
                    👉 <strong style={{ color: '#f1f5f9' }}>
                      {tossWinnerId === match.team1Id ? match.team1Name : match.team2Name}
                    </strong> will {tossDecision === 'bat' ? 'bat' : 'bowl'} first. <strong style={{ color: '#f1f5f9' }}>
                      {tossWinnerId === match.team1Id ? match.team2Name : match.team1Name}
                    </strong> will {tossDecision === 'bat' ? 'bowl' : 'bat'} first. (automatic)
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8, marginTop: 20 }}>
                  <button className="team-btn team-btn--ghost" style={{ flex: 1 }} onClick={() => setTossStep(0)}>← Back</button>
                  <button
                    id="toss-confirm-btn"
                    className="team-btn team-btn--primary"
                    style={{ flex: 2 }}
                    disabled={!tossDecision || tossStartLoading}
                    onClick={handleTossConfirm}
                  >
                    {tossStartLoading ? <><span className="team-spinner" /> Starting…</> : '▶ Confirm & Start Match'}
                  </button>
                </div>
              </div>
            )}
